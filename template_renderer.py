...
def _scene_formula_derivation(scene: dict, idx: int) -> str:
    """[DEPRECATED] Now uses vertical appending to satisfy user requirement."""
    return _scene_formula_step_list(scene, idx)


def _scene_formula_step_list(scene: dict, idx: int) -> str:
    """Appends math formulas vertically on the screen (Derivation History style)."""
    d = scene["visual_data"]
    heading = d.get("heading", "")
    steps = d.get("steps", []) # No more cap, sliding window handles overflow
    duration = d.get("duration", 8.0)
    
    if not steps:
        return f"        # Scene {idx}: formula_step_list (Skipped — no steps provided)\n"
    
    num_steps = len(steps)
    lines = [f"\n        # Scene {idx}: formula_step_list (Sliding Overflow)"]
    lines.append(f"        self._clear()")
    ...
    for i, step in enumerate(steps):
        ...
        if i > 0:
            # Create arrow to pass to overflow handler (this arrow belongs ABOVE the current formula)
            lines.append(f"        {a_vname} = MathTex(r\"\\downarrow\", tex_template=my_template).scale(0.7).set_color(DesignTokens.GRAY)")
            lines.append(f"        self._add_formula_with_overflow({f_vname}, arrow={a_vname})")
        else:
            # First one has no arrow above it
            lines.append(f"        self._add_formula_with_overflow({f_vname})")

    lines.append(f"        self.wait(2.0)")
    lines.append(f"        self._clear()")
    return "\n".join(lines)
...
_GENERATORS = {
    ...
    "formula_derivation": _scene_formula_step_list,
    "formula_step_list": _scene_formula_step_list,
...
def build_manim_script(
    ...
    script = f'''print(f"   🚀 TEMPLATE_RENDERER LOADED FROM: {{__file__}}")
